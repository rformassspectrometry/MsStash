# Authors and Citation

## Authors

- **Johannes Rainer**. Author, maintainer.
  [](https://orcid.org/0000-0002-6977-7147)

- **Philippine Louail**. Author.
  [](https://orcid.org/0009-0007-5429-6846)

- **Laurent Gatto**. Contributor.
  [](https://orcid.org/0000-0002-1520-2268)

## Citation

Source:
[`DESCRIPTION`](https://github.com/RforMassSpectrometry/MsStash/blob/main/DESCRIPTION)

Rainer J, Louail P (2026). *MsStash: Infrastructure to serialize and
restore mass spectrometry data objects*. R package version 0.97.0,
<https://github.com/RforMassSpectrometry/MsStash>.

    @Manual{,
      title = {MsStash: Infrastructure to serialize and restore mass spectrometry data objects},
      author = {Johannes Rainer and Philippine Louail},
      year = {2026},
      note = {R package version 0.97.0},
      url = {https://github.com/RforMassSpectrometry/MsStash},
    }
